...
static struct tee_shm *handle_rpc_alloc(struct tee_tz *ptee, size_t size)
{
	...
	rpc_alloc.size = size;
	tee_supp_cmd(ptee->tee, TEE_RPC_ICMD_ALLOCATE, &rpc_alloc, sizeof(rpc_alloc));
...
#ifdef TKCORE_KDBG
#ifdef CONFIG_ARM64
		pr_debug("%s TEESMC_RPC_FUNC_ALLOC_ARG size 0x%llx\n", __func__, param->a1);
#else
...
#ifdef TKCORE_KDBG
#ifdef CONFIG_ARM64
		pr_debug("%s TEESMC_RPC_FUNC_ALLOC_ARG arg: 0x%llx\n", __func__, param->a1);
#else
...
static int tz_open(struct tee_session *sess, struct tee_cmd *cmd)
{
	...
	if (ta) {
		params32[1].u.memref.buf_ptr =
			tee_shm_pool_v2p(DEV, ptee->shm_pool, cmd->ta->resv.kaddr);
 ...
 *	| TEE_SHM_TEMP | TEE_SHM_FROM_RPC)) != 0 ) {
	...
	if (unlikely(size_aligned == 0)) {
		pr_err("[%s] requested size too big\n", __func__);
		return NULL;
	}
	...
	if (!shm->resv.paddr) {
		pr_err("%s: cannot alloc memory, size 0x%lx\n",
			__func__, (unsigned long)shm->size_alloc);
		devm_kfree(tee->dev, shm);
		return ERR_PTR(-ENOMEM);
	}
	shm->resv.kaddr = tee_shm_pool_p2v(tee->dev, ptee->shm_pool, shm->resv.paddr);
	if (!shm->resv.kaddr) {
		pr_err("%s: p2v(%p)=0\n", __func__, (void *) (unsigned long) shm->resv.paddr);
		tee_shm_pool_free(tee->dev, ptee->shm_pool, shm->resv.paddr, NULL);
		devm_kfree(tee->dev, shm);
	...
	dev_dbg(tee->dev, "%s: kaddr=%p, paddr=%p, shm=%p, size %x:%x\n",
		__func__, shm->resv.kaddr, (void *) (unsigned long) shm->resv.paddr, shm,
...
static void tz_free(struct tee_shm *shm)
{
	...
	ret = tee_shm_pool_free(tee->dev, ptee->shm_pool, shm->resv.paddr, &size);
	if (!ret) {
#ifdef TKCORE_KDBG
...
static int register_outercache_mutex(struct tee_tz *ptee, bool reg)
{
	...
	if ((vaddr = tee_map_cached_shm(paddr, sizeof(u32))) == NULL) {
...
static int configure_shm(struct tee_tz *ptee)
{
	...
	if (param.a0 != TEESMC_RETURN_OK) {
		pr_err("shm service not available: 0x%x", (uint) param.a0);
		ret = -EINVAL;
		goto out;
	}
...
static int tz_tee_init(struct platform_device *pdev)
{
	...
	if (ret) {
		pr_err("%s: dev=%s, Secure failed (%d)\n",
