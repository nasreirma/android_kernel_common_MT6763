...
#ifdef TKCORE_KDBG
	pr_info("%s > tgid:[%d] id:[0x%08x]\n", __func__, task->tgid, id);
#endif

	if (id == TEE_RPC_ICMD_INVOKE) {
		if (atomic_read(&rpc->used) == 0) {
			pr_err("%s: teed not ready. id=0x%x\n" , __func__, id);
			goto out;
		}
	}
	...
	case TEE_RPC_ICMD_ALLOCATE:
		{
			...
			alloc->size = size;
			alloc->data = (void *) (unsigned long) shmint->resv.paddr;
			alloc->shm = shmint;
			res = TEEC_RPC_OK;
		  ...
		  size_t length, loff_t *offset)
{
	...
	if (ctx == NULL || ctx->tee == NULL || ctx->tee->dev == NULL || ctx->tee->rpc == NULL) {
	...
	if (atomic_read(&rpc->used) == 0) {
		pr_err("%s: teed application not ready\n", __func__);
		ret = -EPERM;
		goto out;
	}
	...
	if (length < ret) {
		...
		if (copy_to_user(buffer, &rpc->commToUser, ret)) {
			pr_err("%s: copy_to_user(comm) failed!\n", __func__);
			ret = -EINVAL;
		}
	}
		...
		size_t length, loff_t *offset)
{
...
#ifdef TKCORE_KDBG
	pr_debug("%s > tgid:[%d:%d]\n", __func__, task->tgid, task->pid);
#endif

	if (atomic_read(&rpc->used) == 0) {
		pr_err("%s: teed not ready\n", __func__);
		goto out;
	}
	...
	if (length > 0 && length < sizeof(rpc->commFromUser)) {
		...
		if ((r = copy_from_user(&rpc->commFromUser, buffer, length))) {
			pr_err("%s: copy_from_user(comm) failed: %lu\n", __func__, r);
		...
		for (i = 0; i < rpc->commFromUser.nbr_bf; i++) {
			...
			if (type & TEE_RPC_BUFFER_NONSECURE) {
				...
				struct tee_shm *shm;
				struct vm_area_struct *vma = find_vma(current->mm, (unsigned long) buffer);
				...
				if (shm == NULL) {
					pr_err("Invalid vma->vm_private_data [%s:%d:%d]\n", current->comm, current->tgid, current->pid);
				...
				rpc->commFromUser.cmds[i].buffer = (void *) (unsigned long) shm->resv.paddr;
			}
		}
...
int tee_supp_init(struct tee *tee)
{
	...
	if ((rpc = devm_kzalloc(tee->dev, sizeof(struct tee_rpc), GFP_KERNEL)) == NULL) {
		pr_err("%s: bad devm_kzalloc(rpc)\n", __func__);
		return -ENOMEM;
	}
