...
static void tee_keep_awake(void)
{
	while (atomic_inc_unless_negative(&tee_wakeup_cnt) == 0) {
		wait_event_freezable(suspend_done, atomic_read(&tee_wakeup_cnt) >= 0);
	}
}
...
static void tee_prepare_suspend(void)
{
	...
	pr_warn("%s() tee_wakeup_cnt unexpected value: %d\n", __func__, r);
}
...
static int tee_pm_suspend_notifier(struct notifier_block *nb, unsigned long event, void *dummy)
{
	switch (event) {
	case PM_SUSPEND_PREPARE:
		...
		/*
		{
			pr_warning("tee_prepare_suspend() failed with %d\n", r);
			return NOTIFY_BAD;
		}
		*/
#ifdef CONFIG_TRUSTKERNEL_TEE_RPMB_SUPPORT
...
int tkcore_tee_pm_init(void)
{
	...
	if ((r = register_pm_notifier(&tz_pm_notifier))) {
...
void tkcore_tee_pm_exit(void)
{
	...
	if ((r = unregister_pm_notifier(&tz_pm_notifier))) {
