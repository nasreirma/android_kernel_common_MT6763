...
#define BG_Ratio_Typical 301
static struct OV5695_otp_struct OV5695_OTP = {0};
int OV5695_read_OTP(struct OV5695_otp_struct *otp_ptr)
{
        ...
        printk("otp flag module_id lens_id rg_ratio bg_ratio 0x%x 0x%x 0x%x 0x%x 0x%x\n",(*otp_ptr).flag,(*otp_ptr).module_integrator_id,(*otp_ptr).lens_id,(*otp_ptr).rg_ratio,(*otp_ptr).bg_ratio);
        for(i=0x700C;i<=0x701B;i++)
        {
            write_cmos_sensor(i,0x0); //clear OTP buffer
        }

        return(*otp_ptr).flag;
}

int OV5695_apply_OTP(struct OV5695_otp_struct *otp_ptr)
{
...
static kal_uint32 open(void)
{
  ...
  write_cmos_sensor(0x0100,0x01);
  OV5695_read_OTP(&OV5695_OTP);	
  OV5695_apply_OTP(&OV5695_OTP);
