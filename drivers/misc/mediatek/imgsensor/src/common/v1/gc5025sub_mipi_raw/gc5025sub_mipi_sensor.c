...
#include "gc5025sub_mipi_sensor.h"
#include "gpio/gpio.h"
...
//#define GC5025SUBOTP_FOR_CUSTOMER

static DEFINE_SPINLOCK(imgsensor_drv_lock);
static kal_bool GC5025SUBDuringTestPattern = KAL_FALSE;

static kal_uint32 Dgain_ratio = 1;
static kal_bool DR_State = KAL_TRUE;

static imgsensor_info_struct imgsensor_info = {
       .sensor_id = GC5025SUB_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
#ifdef GC5025SUBOTP_FOR_CUSTOMER
...
{	
    kal_uint8 flag_doublereset = 0,flag_GC5025SUBA = 0;
    ...
    flag_GC5025SUBA = gc5025_read_otp(0x27);

	if((flag_GC5025SUBA & 0x01) == 0x01) {
		LOG_INF("GC5025SUBA sensor! \n");
		DR_State = KAL_FALSE;
    }
	else { 
		if((flag_doublereset & 0x03)==0x01) {
			DR_State = KAL_FALSE;
			LOG_INF("GC5025SUB sensor double reset off \n");
		} else {
			DR_State = KAL_TRUE;
			LOG_INF("GC5025SUB sensor double reset on \n");
		}
    }
...
UINT32 GC5025SUB_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
