...
static imgsensor_info_struct imgsensor_info = { 
    ...
    .pre = {
        ...
        .linelength = 5528,                   //record different mode's linelength
        .framelength = 1688,            //record different mode's framelength
...
static void write_shutter(kal_uint16 shutter)
{
    ...
    // Update Shutter
    //// 4h7yx write_cmos_sensor(0x0104, 0x01);   //group hold
...
static void preview_setting(void)
{
    ...
    write_cmos_sensor_8(0x030F, 0xE1);
    ...
    write_cmos_sensor_8(0x0114, 0x01);  //2LANE
    ...
    write_cmos_sensor_8(0x0343, 0x98);
    write_cmos_sensor_8(0x0200, 0x15);
    write_cmos_sensor_8(0x0201, 0x08);
    ...
    while(imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
            printk("s5k4h7mipiraw_Sensor get_imgsensor_id *sensor_id = %x\r\n",*sensor_id);
            if (*sensor_id == imgsensor_info.sensor_id) {


                printk("s5k4h7_yx  i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);   
                return ERROR_NONE;
                //break;
            } 
            printk("s5k4h7_yx  read sensor id fail, id: 0x%x\n",*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
            printk("s5k4h7mipiraw_Sensor open sensor_id = %x\r\n",sensor_id);
            if (sensor_id == imgsensor_info.sensor_id) {    
