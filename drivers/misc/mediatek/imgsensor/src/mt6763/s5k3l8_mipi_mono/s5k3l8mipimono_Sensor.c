...
#include "s5k3l8mipimono_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "S5K3L8MONO"
#define LOG_1 LOG_INF("S5K3L8MONO,MIPI 4LANE\n")
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K3L8_MONO_SENSOR_ID,		//Sensor ID Value: 0x30C8//record sensor id defined in Kd_imgsensor.h
...
static void set_shutter_frame_length(kal_uint16 shutter, kal_uint16 frame_length)
{
    ...
    imgsensor.shutter = shutter;
    spin_unlock_irqrestore(&imgsensor_drv_lock, flags);
	LOG_INF(" set_shutter_frame_length shutter =%d, frame_length =%d\n", shutter, frame_length);
    ...
    // Update Shutter
	write_cmos_sensor(0x0104, 0x01);
	write_cmos_sensor(0x0350, 0x01); /* Disable auto extend */
	write_cmos_sensor(0x0202, shutter & 0xFFFF);
	...
	//write_cmos_sensor(0x30BE, 0x0004); 
...
{
  LOG_INF(" sensor_init_mono  lpl E\n");
    write_cmos_sensor(0x6028, 0x4000);
...
//$MV1[MCLK:24,Width:2104,Height:1560,Format:MIPI_Raw10,mipi_lane:4,mipi_datarate:1124,pvi_pclk_inverse:0]
  ...
  write_cmos_sensor(0x316A, 0x00A0);
	...
	//dual
	//LOG_INF("custom2_setting mono dual on lpl E\n");
	//write_cmos_sensor(0x6028, 0x2000); 
	//write_cmos_sensor(0x602A, 0x4908); 
	//write_cmos_sensor(0x6F12, 0x0100); 
	//write_cmos_sensor(0x602A, 0x4910); 
...
{
	LOG_INF("E! capture_setting lpl mono currefps:%d\n",currefps);
	if (currefps == 300) {
  ...
  write_cmos_sensor(0x316A, 0x00A0);
	...
	//dual
	//LOG_INF("capture_setting mono dual off lpl E\n");
	//write_cmos_sensor(0x6028, 0x4000);
	//write_cmos_sensor(0x30BE, 0x0000);
	//dual
...
static kal_uint32 get_info(MSDK_SCENARIO_ID_ENUM scenario_id,
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	LOG_INF("get_info mono scenario_id = %d\n", scenario_id);
...
UINT32 S5K3L8_MIPI_MONO_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	S5K3L8_MIPI_MONO_SensorInit	*/
