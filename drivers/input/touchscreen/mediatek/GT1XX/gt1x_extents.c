...
/* Vanzo:wangfei on: Sat, 23 Apr 2016 14:09:12 +0800
 * added by wf for wakeup hung bug
 */
extern struct wake_lock gesture_suspend_lock;
	...
	mutex_unlock(&gesture_data_mutex);
        ...
        switch (doze_buf[0]){
            ...
            case 0x5E:
                ...
                break;
            default:
                key_code = KEY_GES_REGULAR;
                break;
        }
        //gesture_doze_status = DOZE_WAKEUP;
	GTP_DEBUG("Gesture: 0x%02X, points: %d", doze_buf[0], doze_buf[1]);

/* Vanzo:wangfei on: Sat, 23 Apr 2016 14:09:25 +0800
 * added by wf for wakeup hung
 */
    wake_lock_timeout(&gesture_suspend_lock, 200*HZ);
// End of Vanzo:wangfei
...
/* Vanzo:wangfei on: Sat, 23 Apr 2016 14:09:49 +0800
 * added b y wf for wakeup hung
 */
    msleep(200);
// End of Vanzo:wangfei
...
{
    printk("kls gesture_clear_wakeup_data\n");
	mutex_lock(&gesture_data_mutex);
