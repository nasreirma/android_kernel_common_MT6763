...
static ssize_t gt1xx_gesture_show(struct device *dev,struct device_attribute *attr,char *buf)
{
...
static ssize_t gt1xx_gesture_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
    if(count == 0)
        return;
#ifdef GTP_CONFIG_OF
...
static ssize_t gt1xx_fwversion_show(struct device *dev,struct device_attribute *attr,char *buf)
{
  ssize_t num_read_chars = 0;
  s32 ret = -1;
  ...
  u8 id[5]={ 0 };
  u16 product = 0;
  memcpy(id, gt1x_version.product_id, 4);
  product = ((id[0]&0x0F)<<12 | (id[1]&0x0F)<<8 |(id[2]&0x0F)<<4 |(id[3]&0x0F));

  ret = gt1x_i2c_read(GTP_REG_VERSION, reg, sizeof(reg));

  if (ret < 0)
  {
    num_read_chars = snprintf(buf, PAGE_SIZE, "Firmware version can't read!!!\n");
    GTP_DEBUG("version not found !");
    return num_read_chars;
  }

  ret = gt1x_i2c_read(GTP_REG_CONFIG_DATA, config_reg, sizeof(config_reg));
  ...
  num_read_chars = snprintf(buf, PAGE_SIZE, "IC VERSION:GT%x_%02x%02x%02x_V%d\n",product, reg[4], reg[5], reg[6], config_reg[0]);
  //num_read_chars = snprintf(buf, PAGE_SIZE, "IC VERSION:V%d\n", config_reg[0]);
  return num_read_chars;

}

static DEVICE_ATTR(gesture, 0664, gt1xx_gesture_show, gt1xx_gesture_store);
static DEVICE_ATTR(fwversion, 0755, gt1xx_fwversion_show, NULL);

static struct attribute *gt1xx_attributes[] = {
...
static struct attribute_group gt1xx_attribute_group = {
    .attrs = gt1xx_attributes
};
...
#endif
        if ((ret = sysfs_create_group(&client->dev.kobj, &gt1xx_attribute_group))) {
