...
{
    u8 project_id[10], color_id[2], temp[12];
...
//        gpio_set_value(MS_TS_MSG_IC_GPIO_RST, 1); 
    ...
    /*if (MS_TS_MSG_IC_GPIO_RST >= 0 && MS_TS_MSG_IC_GPIO_RST != 1) // MS_TS_MSG_IC_GPIO_RST must be a value other than 1
    {
        ...
        tpd_gpio_output(GPIO_MSG_RST, 1);
        mdelay(10);
        tpd_gpio_output(GPIO_MSG_RST, 0);
        mdelay(10);
        tpd_gpio_output(GPIO_MSG_RST, 1);
        mdelay(25);
#else
...
void DrvCheckFWSupportDriver(void)
{
    ...
    if((szDriverVer[0] << 24) + (szDriverVer[1] << 16) + (szDriverVer[2] << 8) + szDriverVer[3] > 
